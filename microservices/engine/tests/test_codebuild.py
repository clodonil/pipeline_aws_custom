...
class TestCodeBuild:
    ...
    def test_deve_retornar_um_codebuild_customizado_com_imagem_definica(self, params):
        ...
        codeobject = libcodebuild.create_codebuild(
            'Testecodebuild', params['nome'], params['env'], imagecustom='python', buildspec=False)
    ...
    def test_deve_retornar_um_codebuild_customizado_sem_imagem_definica(self, params):
        ...
        image_com_espaco = codebuild.create_codebuild(
            'Testecodebuild', params['nome'], params['env'], imagecustom="", buildspec=False,)
        image_com_false = codebuild.create_codebuild(
            'Testecodebuild', params['nome'], params['env'], imagecustom=False, buildspec=False)
        ...
        assert 'aws/codebuild/standard:2.0' == code_com_espaco['Resources'][
            'Testecodebuild']['Properties']['Environment']['Image']
        assert 'aws/codebuild/standard:2.0' == code_com_false['Resources'][
            'Testecodebuild']['Properties']['Environment']['Image']

    def test_deve_retornar_um_codebuild_customizado_com_cache_definido(self, params):
        codebuild = NewCodeBuild(params['role'])
        codebuild_com_buildspec = codebuild.create_codebuild(
            'Testecodebuild', params['nome'], params['env'], cache=True)
        ...
        assert '${AWS::AccountId}-cache' in code_buildspec['Resources'][
            'Testecodebuild']['Properties']['Cache']['Location']['Fn::Sub']

    def test_deve_retornar_um_codebuild_customizado_sem_cache_definido(self, params):
        codebuild = NewCodeBuild(params['role'])
        codebuild_com_buildspec = codebuild.create_codebuild(
            'Testecodebuild', params['nome'], params['env'], cache=False)
    ...
    def test_deve_retornar_um_codebuild_customizado_com_buildspec_definido(self, params):
        codebuild = NewCodeBuild(params['role'])
        codebuild_com_buildspec = codebuild.create_codebuild(
            'Testecodebuild', params['nome'], params['env'], imagecustom=False, buildspec='test_buildspec.yml')

        code_buildspec = self.gerando_cloudformation(codebuild_com_buildspec)
        print(code_buildspec)
        assert "test_buildspec.yml" in code_buildspec['Resources'][
            'Testecodebuild']['Properties']['Source']['BuildSpec']

    def test_deve_retornar_um_codebuild_customizado_sem_buildspec_definido(self, params):
        codebuild = NewCodeBuild(params['role'])
        codebuild_com_buildspec_espaco = codebuild.create_codebuild(
            'Testecodebuild', params['nome'], params['env'], imagecustom=False, buildspec='')
        codebuild_com_buildspec_false = codebuild.create_codebuild(
            'Testecodebuild', params['nome'], params['env'], imagecustom=False, buildspec=False)

        buildspec_espaco = self.gerando_cloudformation(
            codebuild_com_buildspec_espaco)
        buildspec_false = self.gerando_cloudformation(
            codebuild_com_buildspec_false)
        assert "pipeline/buildspec_testecodebuild.yml" in buildspec_espaco[
            'Resources']['Testecodebuild']['Properties']['Source']['BuildSpec']
        assert "pipeline/buildspec_testecodebuild.yml" in buildspec_false[
            'Resources']['Testecodebuild']['Properties']['Source']['BuildSpec']
    ...
    def test_deve_retornar_um_codebuild_customizado_sem_env_definido(self, params):
        libcodebuild = NewCodeBuild(params['role'])
        codeobject = libcodebuild.create_codebuild(
            'Testecodebuild', params['nome'], '', imagecustom=False, buildspec=False)
        codeobject_vazio = libcodebuild.create_codebuild(
            'Testecodebuild', params['nome'], [], imagecustom=False, buildspec=False)

        codebuild = self.gerando_cloudformation(codeobject)
        codebuild_vazio = self.gerando_cloudformation(codeobject_vazio)

        assert len(codebuild['Resources']['Testecodebuild']
                   ['Properties']['Environment']['EnvironmentVariables']) == 0
        assert len(codebuild_vazio['Resources']['Testecodebuild']
                   ['Properties']['Environment']['EnvironmentVariables']) == 0

    def test_deve_retornar_um_codebuild_customizado_com_env_definido_errado(self, params):
        libcodebuild = NewCodeBuild(params['role'])
        codeobject = libcodebuild.create_codebuild('Testecodebuild', params['nome'], [
                                                   '10', '20'], imagecustom=False, buildspec=False)
        codeobject_dict = libcodebuild.create_codebuild('Testecodebuild', params['nome'], [
                                                        {'var1': 'var1'}, {'var2': 'var2'}], imagecustom=False, buildspec=False)
        codeobject_dict_name = libcodebuild.create_codebuild('Testecodebuild', params['nome'], [
                                                             {'Name': 'var1'}, {'Name': 'var2'}], imagecustom=False, buildspec=False)
        codeobject_dict_value = libcodebuild.create_codebuild('Testecodebuild', params['nome'], [
                                                              {'Value': 'var1'}, {'Value': 'var2'}], imagecustom=False, buildspec=False)
        codebuild = self.gerando_cloudformation(codeobject)
        ...
        codebuild_dict_value = self.gerando_cloudformation(
            codeobject_dict_value)

        assert len(codebuild['Resources']['Testecodebuild']
                   ['Properties']['Environment']['EnvironmentVariables']) == 0
        assert len(codebuild_dict['Resources']['Testecodebuild']
                   ['Properties']['Environment']['EnvironmentVariables']) == 0
        assert len(codebuild_dict_name['Resources']['Testecodebuild']
                   ['Properties']['Environment']['EnvironmentVariables']) == 0
        assert len(codebuild_dict_value['Resources']['Testecodebuild']
                   ['Properties']['Environment']['EnvironmentVariables']) == 0

    def test_deve_retornar_um_codebuild_customizado_quando_nome_codebuild_tiver_alphanumerico(self, params):
        libcodebuild = NewCodeBuild(params['role'])
        codeobject = libcodebuild.create_codebuild(
            'TesteCodebuild', 'Teste-codebuild', params['env'], imagecustom='python', buildspec=False)
        codeobject_ = libcodebuild.create_codebuild(
            'TesteCodebuild', 'Teste_codebuild', params['env'], imagecustom='python', buildspec=False)
        codeobject__ = libcodebuild.create_codebuild(
            'TesteCodebuild', 'Teste$codebuild', params['env'], imagecustom='python', buildspec=False)
    ...
    def test_deve_retornar_codebuild_do_Controlversion(self, params, imageCustom):
        for runtime in params['runtime']:
            ...
            codebuild = newcodebuild.Controlversion(
                featurename='pipeline', microservicename='teste', runtime=runtime, branchname='develop', custom=False, imageCustom=imageCustom)
            ...
            assert 'common/controlversion/buildspec.yml' in cf['Resources'][
                'Controlversion']['Properties']['Source']['BuildSpec']
    ...
    def test_deve_retornar_codebuild_do_Sast(self, params, imageCustom):
        for runtime in params['runtime']:
            newcodebuild = NewCodeBuild(params['role'])
            sast = newcodebuild.Fortify(featurename='pipeline', microservicename='teste',
                                        runtime=runtime, branchname='develop', custom=False, imageCustom=imageCustom)
    ...
    def test_deve_retornar_codebuild_do_Testunit_sem_builcustomizado(self, params, imageCustom):
        for runtime in params['runtime']:
            ...
            codebuild = newcodebuild.Testunit(featurename='pipeline', microservicename='teste',
                                              runtime=runtime, branchname='develop', custom=False, imageCustom=imageCustom)
            cf = self.gerando_cloudformation(codebuild)
            print(cf)
            assert 'pipeline-teste-testunit-develop' in cf['Resources']['Testunit']['Properties']['Name']
            assert f"../01/{runtime}/testunit/buildspec.yml" in cf[
    ...
    def test_deve_retornar_codebuild_do_Sonar(self, params, imageCustom):
        for runtime in params['runtime']:
            ...
            assert f'../01/{runtime}/sonarqube/buildspec.yml' in cf[
                'Resources']['Sonar']['Properties']['Source']['BuildSpec']
    ...
    def test_deve_retornar_codebuild_do_Aqua(self, params, imageCustom):
        newcodebuild = NewCodeBuild(params['role'])
        codebuild = newcodebuild.Aqua(runtime='python', featurename='pipeline',
                                      microservicename='teste', branchname='develop', custom=True, imageCustom=imageCustom)
    ...
    def test_deve_retornar_codebuild_do_deploy_ecs(self, params, imageCustom):
        newcodebuild = NewCodeBuild(params['role'])
        codebuild = newcodebuild.Deployecsdev(
            runtime='python', featurename='pipeline', microservicename='teste', branchname='develop', imageCustom=imageCustom)
        cf = self.gerando_cloudformation(codebuild)
        print(cf)
        assert 'pipeline-teste-deployecsdev-develop' in cf['Resources']['Deployecsdev']['Properties']['Name']
    ...
    def test_deve_retornar_codebuild_do_Publish_ECR(self, params, imageCustom):
        newcodebuild = NewCodeBuild(params['role'])
        codebuild = newcodebuild.Publishecrdev(
            runtime='python', featurename='pipeline', microservicename='teste', branchname='develop', imageCustom=imageCustom)

        cf = self.gerando_cloudformation(codebuild)
        print(cf)
        assert 'pipeline-teste-publishecrdev-develop' in cf['Resources']['Publishecrdev']['Properties']['Name']
